...
final class Cachify {
	...
	private static function _set_default_vars() {
		...
		if ( self::METHOD_APC === self::$options['use_apc'] && Cachify_APC::is_available() ) {
			...
			/* REDIS */
		} elseif ( self::METHOD_RED === self::$options['use_apc'] && Cachify_REDIS::is_available() ) {
	...
	private static function _method_select() {
		...
		$methods = array(
			...
			self::METHOD_RED => esc_html__( 'Redis', 'cachify' ),
		);
	...
	private static function _get_tabs( $options ) {
		...
		} elseif ( self::METHOD_RED === $options['use_apc'] && self::$is_nginx ) {
			/* Setup tab for Redis */
			$tabs['setup'] = array(
				'name' => __( 'Setup', 'cachify' ),
				'page' => CACHIFY_DIR . '/views/setup/redis-nginx.php',
			);
		}
